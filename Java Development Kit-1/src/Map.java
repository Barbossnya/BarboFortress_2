...
public class Map extends JPanel {
    ...
    private void update(MouseEvent mouseEvent) {
        ...
        field[y][x] = HUMAN_DOT; // Изменили положение на поле для игрока
        if (checkEndGame(HUMAN_DOT, STATE_WIN_HUMAN)) {
            return; // Если игрок выиграл, выйти из метода
        }
        aiTurn();
        repaint();
    ...
    private boolean checkWin(int dot) {
        for (int i = 0; i < fieldSizeY; i++) {
            for (int j = 0; j < fieldSizeX; j++) {
                if (checkLine(i, j, 1, 0, winLen, dot))
                    return true;
                if (checkLine(i, j, 1, 1, winLen, dot))
                    return true;
                if (checkLine(i, j, 0, 1, winLen, dot))
                    return true;
                if (checkLine(i, j, 1, -1, winLen, dot))
                    return true;
            }
    ...
    private boolean checkLine(int x, int y, int vx, int vy, int len, int dot) {
        ...
        return true; // Возвращаем true, если линия заполнена нужным значением
    }
        ...
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                ...
                if (field[y][x] == HUMAN_DOT) {
                    g.drawLine(x * cellWidth + PADDING, y * cellHeight + PADDING, (x + 1) * cellWidth - PADDING,
                            (y + 1) * cellHeight - PADDING);
                    g.drawLine(x * cellWidth + PADDING, (y + 1) * cellHeight - PADDING, (x + 1) * cellWidth - PADDING,
                            y * cellHeight + PADDING);
                } else if (field[y][x] == AI_DOT) {
                    g.drawOval(x * cellWidth + PADDING, y * cellHeight + PADDING, cellWidth - PADDING * 2,
    ...
    private void showMessage(Graphics g) {
        ...
        switch (gameStateType) {
            ...
            default -> throw new RuntimeException("Uchecked gameOverState: " + gameStateType);
        }
        gameWork = false;
    }
}